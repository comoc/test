...
jQuery.glProgram = function( gl, config, complete )
{
  ...
  if (config.VertexProgramURL)
  	$.get( config.VertexProgramURL,   function( source ) { LoadVertexShader.apply( that, arguments ); } );
  if (config.FragmentProgramURL)
	  $.get( config.FragmentProgramURL, function( source ) { LoadFragmentShader.apply( that, arguments ); } );
 
  if (config.VertexProgramSource)
	  LoadVertexShader( config.VertexProgramSource );
  if (config.FragmentProgramSource)
	  LoadFragmentShader( config.FragmentProgramSource );
}
...
jQuery.glModel = function( gl, model_url, complete )
{
  ...
  $.get( model_url, function( source ) { LoadModel.apply( that, arguments ); }, "json" );
}
...
jQuery.glTexture = function( gl, config, complete )
{
  ...
  var ImageLoaded = function( image, image_type )
  {
    gl.bindTexture( texture_type, this.Texture );
    $.glCheckError( gl, this.ErrorLog );
    ...
    gl.texImage2D( image_type, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, image );
    $.glCheckError( gl, this.ErrorLog );
  ...
  var LoadImage = function( src, image_type )
  {
    var image        = new Image();
    image.onload     = function() { ImageLoaded.apply( that, [ image, image_type ] ); };
    image.src        = src;
  }
  ...
  {
    images_remaining = 1;
    LoadImage( config.ImageURL, gl[ config.Type ] );
  }
  else
  {
    ...
    {
      LoadImage( config.ImageURL[i], gl[ config.ImageType[i] ] );
    }
  }
}
